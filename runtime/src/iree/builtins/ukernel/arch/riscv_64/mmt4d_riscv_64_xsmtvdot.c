...
// Move one 4x4 atom (16 contiguous row-major int32 lanes per accumulator pair)
// between the atom-packed scratch buffer and the row-major `out` tile (row
// stride N0). Runs once per call, outside the K reduction loop, so it has no
// impact on inner-loop throughput.
//
//   scratch[(mt*NT + nt)*16 + r*4 + c]  <->  out[(mt*4 + r)*N0 + (nt*4 + c)]
static void iree_uk_ime_gather_acc(const iree_uk_int32_t* out,
...
static void iree_uk_ime_scatter_acc(iree_uk_int32_t* out,
...
//   A atoms: v0, v2, v4
//   B atoms: v1, v3, v5, v6
//   acc    : 12 pairs {v8,v10,v12,v14,v16,v18,v20,v22,v24,v26,v28,v30}
...
iree_uk_mmt4d_tile_s8s8s32_12x16x8_riscv_64_xsmtvdot(
  ...
  // This kernel implements exactly the 12x16x8 IME macro-tile.
  ...
  for (int k1 = 0; k1 < K1; ++k1) {
    const iree_uk_int8_t* A0 =
        lhs + k1 * (M0_T * IME_K0) + 0 * IME_M_ATOM * IME_K0;
    const iree_uk_int8_t* A1 =
        lhs + k1 * (M0_T * IME_K0) + 1 * IME_M_ATOM * IME_K0;
    const iree_uk_int8_t* A2 =
        lhs + k1 * (M0_T * IME_K0) + 2 * IME_M_ATOM * IME_K0;
    const iree_uk_int8_t* B0 =
        rhs + k1 * (N0_T * IME_K0) + 0 * IME_N_ATOM * IME_K0;
    const iree_uk_int8_t* B1 =
        rhs + k1 * (N0_T * IME_K0) + 1 * IME_N_ATOM * IME_K0;
    const iree_uk_int8_t* B2 =
        rhs + k1 * (N0_T * IME_K0) + 2 * IME_N_ATOM * IME_K0;
    const iree_uk_int8_t* B3 =
        rhs + k1 * (N0_T * IME_K0) + 3 * IME_N_ATOM * IME_K0;
